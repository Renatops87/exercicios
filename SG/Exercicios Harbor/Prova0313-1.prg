...
do while .t.
   ...
   do while nContador < nNumeroEmpregados
      ...
      @ 14,02 say "Valor do ticket de alimentacao: "
      ...
      @ 14,33 get nValorTicketAlimentacao      picture "@E 999,999.9"       valid nValorTicketAlimentacao >= 0
      read
      ...
      nTotalTrabalhado := nAnoDemissao - nAnoAdmissao

      nValorAdicionalNoturno := nValorSalarioBase * (nPercentualAdicionalNoturno / 100) // adicional noturno
      nValorInsalubridade    := nValorSalarioBase * (nPercentualInsalubridade / 100) // insalubridade

      // caso tenha trabalhado entre 2005 e 2006
      if nAnoDemissao > 2005 .and. nAnoDemissao > 2006  // 2005
      ...
      nValorSalarioBruto := nValorSalarioBase + nValorAdicionalNoturno + nValorInsalubridade

      // mulher com IRRF
      if cSexoColaborador == "F" .and. nValorSalarioBruto > nValorLimiteIrrf
         nQuantidadeFcomIrrf++
      endif

      // redutor 11% para salario base > valor limite irrf
      if nValorSalarioBruto > nValorLimiteIrrf
         nReducaoIRRF := nValorSalarioBase * 0.11
      endif

      if nValorTicketAlimentacao > 0
         nReducaoAlimentacao := nValorSalarioBase * 0.20
      endif

      nValorRemuneracao = nValorSalarioBruto - nReducaoIRRF + nAdicional20052006 - nReducao20092010 - nReducaoAlimentacao

      if nIdadeColaborador >= 60 .and. nTotalTrabalhado >= 25 .and. cSexoColaborador == "F"
         nMulherApta++
         nContaAposentado++
         nTotalRemuneracao += nValorRemuneracao + nReducaoAlimentacao
      endif
      ...
      if cSexoColaborador == "M"
         ...
         if nValorTicketAlimentacao > 0
            nQuantidadeHomensTicket++
         endif
         if nIdadeColaborador >= 65 .and. nTotalTrabalhado >= 30
            nHomemApto++
            nContaAposentado++
            nTotalRemuneracao += nValorRemuneracao + nReducaoAlimentacao
         endif
      ...
      @ 16,02 say "Reducao 2009: "          + alltrim(str(nReducao20092010))
      ...
      @ 22,02 say "Valor reducao ticket: "    + alltrim(str(nReducaoAlimentacao))
      nContador++
   ...
   if (nRespostaCancelar == 1)
      ...
      nPercentualHomensTicket := (nQuantidadeHomensTicket / nContadorHomens) * 100
      @ 02,02 say "Total Remuneracao: "                              + alltrim(str(nTotalRemuneracao))
      @ 03,02 say "Quantidade de Aposentados: "                      + alltrim(str(nContaAposentado))
      ...
      @ 09,02 say "% de Homens que recebem ticket de alimentacao : " + alltrim(str(nPercentualHomensTicket))
   endif
